...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        self.q_target = tf.placeholder(tf.float32, [None, self.n_actions + 1], name='Q_target')  # for calculating loss, extra one for staying
        with tf.variable_scope('eval_net'):
            ...
            with tf.variable_scope('l2'):
                self.w2 = tf.get_variable('w2', [n_l1, self.n_actions + 1], initializer=w_initializer, collections=c_names)
                self.b2 = tf.get_variable('b2', [1, self.n_actions + 1], initializer=b_initializer, collections=c_names)
        ...
        with tf.variable_scope('target_net'):
            ...
            # second layer. collections is used later when assign to target net
            with tf.variable_scope('l2'):
                w2 = tf.get_variable('w2', [n_l1, self.n_actions + 1], initializer=w_initializer, collections=c_names)
                b2 = tf.get_variable('b2', [1, self.n_actions + 1], initializer=b_initializer, collections=c_names)
    ...
    ''' choose one of predicates or STAY (end)
    '''
    def choose_action(self, observation_lrhs, selected_rhs, predicatesArr):
        ...
        # add STAY into unselected_new
        unselected_new.append(len(predicatesArr))
