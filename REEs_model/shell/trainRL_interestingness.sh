...
data_dir="/opt/disk1/user/discovery/trainDQN/"


# 1. train the rule interestingness model
echo -e "---------------------------------------------- TRAIN the rule interestingness model -------------------------------------------------------------------"
rules_file=${data_dir}"Interestingness/interestingness_labels/"${task[${tid}]}'/rules.csv'
train_file=${data_dir}"Interestingness/interestingness_labels/"${task[${tid}]}'/train/train.csv'
test_file=${data_dir}"Interestingness/interestingness_labels/"${task[${tid}]}'/train/test.csv'
predicates_path=${data_dir}"/all_predicates/"${task[${tid}]}"_predicates.txt"
interestingness_model_path=${data_dir}"Interestingness/interestingness_model/"${task[${tid}]}"/"
vobs_file=${data_dir}"Interestingness/interestingness_model/"${task[${tid}]}"/tokenVobs.txt"
mkdir ${interestingness_model_path}
interestingness_model_txt_path=${data_dir}"Interestingness/interestingness_txt_model/"${task[${tid}]}"/model.txt"
mkdir ${data_dir}'Interestingness/interestingness_txt_model/'
mkdir ${data_dir}'Interestingness/interestingness_txt_model/'${task[${tid}]}'/'
...
dqn_model_dir=${data_dir}'Interestingness/dqn_interestingness_model/'${task[${tid}]}$'/'
mkdir ${data_dir}'Interestingness/dqn_interestingness_model/'
mkdir ${dqn_model_dir}
...
filter_data_dir=${data_dir}'Interestingness/filter_interestingness_data/'${task[${tid}]}'/'
mkdir ${data_dir}'Interestingness/filter_interestingness_data/'
...
filter_model_dir=${data_dir}'Interestingness/filter_interestingness_model/'${task[${tid}]}'/'
mkdir ${data_dir}'Interestingness/filter_interestingness_model'
