...
class LSHash(object):
    ...
    # 旧版本，逐个点进行转换，未采用位操作。速度比较慢。
    def get_hash_bak(self, input_point, extra_data=None, seed=None):
        ...
        if seed != None and seed != self.seed:
            self.seed = seed
            np.random.seed(seed)
            self.uniform_planes = [self._generate_uniform_planes()
                                   for _ in range(self.num_hashtables)]
        ...
        hashes = []
        for i, table in enumerate(self.hash_tables):
            projections = np.dot(self.uniform_planes[i], input_point)

            for j in range(len(projections)):
                projections[j] = 1 if projections[j] > 0 else 0
            hashes.append(projections)

        # 把哈希得到的二维数组（hash_size * num_hashtables）转成一维数组
        # 把第一维（hash_size大小）的数组中的0-1按位转成整数
        hashes2 = []
        for hash in hashes:
            hash_str = ""
            for num in hash:
                hash_str = hash_str + str(int(num))


            value = BitArray(bin=hash_str)
            hash_bytes = value.tobytes()
            hash_num = int.from_bytes(hash_bytes, byteorder='little', signed=True)
            hashes2.append(hash_num)

        return hashes2

    # 逐个点进行转换，采用位操作。
    def get_hash(self, input_point, seed=None):
        if seed != None and seed != self.seed:
            self.seed = seed
            np.random.seed(seed)
            self.uniform_planes = [self._generate_uniform_planes()
        ...
        projections = np.dot(self.uniform_planes, input_point)

        for i in range(len(projections)):
            projection = projections[i]
            for j in range(len(projection)):
                projections[i][j] = 1 if projections[i][j] > 0 else 0
        hashes = projections
    ...
    def get_hash_for_pointlist(self, input_points, seed=None):
        if seed != None and seed != self.seed:
            self.seed = seed
            np.random.seed(seed)
            self.uniform_planes = [self._generate_uniform_planes()
        ...
        hash_array = []
        arr = np.array(input_points).T

        projections_array = np.dot(self.uniform_planes, arr).T

        for projections in projections_array:
            hashes = []
            ...
            for hash in hashes:
                ...
                hashes2.append(hash_num)
            hash_array.append(hashes2)

        return hash_array

    def get_hash_for_pointlist_multiprocessing(self, input_points, seed=None):
        sys = platform.system()
        if sys == "Windows":
            return self.get_hash_for_pointlist(input_points, seed)
        ...
        for input_point in input_points:
            point_bucket.append(input_point)
            i = i + 1
            if i > bucket_size:
                point_buckets.append(point_bucket)
                point_bucket = []
                i = 0
        ...
        pool = ThreadPool()
        hash_buckets = pool.map(self.get_hash_for_pointlist, point_buckets)
        pool.close()
        pool.join()

        hash_array = []
        for hash_bucket in hash_buckets:
            for hash in hash_bucket:
