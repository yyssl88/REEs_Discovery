...
from unsupervised_learning.dagmm.compression_net import CompressionNet
from unsupervised_learning.dagmm.estimation_net import EstimationNet
from unsupervised_learning.dagmm.gmm import GMM
...
class DAGMM:
    ...
    def fit(self, x):
        ...
        with tf.Graph().as_default() as graph:
            ...
            self.gamma = self.est_net.inference(z, drop)
            self.gmm.fit(z, self.gamma)
            ...
            tf.add_to_collection("save", self.gamma)
    ...
    def restore(self, fdir):
        ...
        with tf.Graph().as_default() as graph:
            ...
            self.input, self.energy, self.gamma = tf.get_collection("save")
