...
import sics.seiois.mlsserver.biz.der.mining.recovery.MultiTuplesRuleMiningConstantRecovery;
import sics.seiois.mlsserver.biz.der.mining.recovery.REETemplate;
...
public class ParallelRuleDiscoverySamplingB {
    // predicate set
    // each predicate attaches corresponding data in ParsedColumn
    private ArrayList<Predicate> allPredicates; // all predicates
    private ArrayList<Predicate> allPredicatesTemplate; // nonconstant predicates + constant predicate templates ONLY <t_0, t_1> and <t_0>
    private ArrayList<Predicate> realAllConstantPredicates; // real constant predicates with t_0, ..., t_{max}
    ...
    public static int MAX_CURRENT_PREDICTES = 10;
    ...
    private void generatePredicatesTemplate(ArrayList<Predicate> predicates) {
        this.allPredicatesTemplate = new ArrayList<>();
        HashSet<String> dups = new HashSet<>();
        for (Predicate p : predicates) {
            if (p.isConstant()) {
                String k = p.getOperand1().getColumn().toStringData();
                if (!dups.contains(k)) {
                    Predicate cpTemplate = predicateProviderIndex.getConstantTemplate(p, 0, 0);
                    this.allPredicatesTemplate.add(cpTemplate);
//                    this.allPredicates.add(cpTemplate);
                    dups.add(k);
                }
            } else {
                this.allPredicatesTemplate.add(p);
            }
        }
    }

    public ParallelRuleDiscoverySamplingB(ArrayList<Predicate> predicates, int maxTupleNum, long support,
        ...
        this.generatePredicatesTemplate(predicates);
        this.realAllConstantPredicates = new ArrayList<>();
...
//        lattice.initialize(this.allPredicates, this.maxTupleNum);
        ...
        // prepare all nonconstant predicates with <t_0, t_1> and <t_0, t_2> and constant templates with <t_0>, <t_1> and <t_2>
        // only for test application-driven methods
        // RHSs predicate: <t_0>, <t_0, t_1>
        ...
        // only consider non-constant predicates and constant predicate templates
//        lattice.initialize(this.allPredicatesTemplate, this.maxTupleNum, applicationRHSs);
...
//            Lattice nextLattice = runNextLattices(lattice, this.allPredicatesTemplate, this.invalidX, this.invalidXRHSs, this.validXRHSs,
//                    currentSupports, predicateProviderIndex, "", null, spark);
    ...
    public Lattice runNextLattices(Lattice lattice, ArrayList<Predicate> allPredicatesTemplate, HashSet<IBitSet> invalidX,
        ...
        int KthScore = 0;
        BroadcastLattice broadcastLattice = new BroadcastLattice(allPredicatesTemplate, invalidX, invalidXRHSs, validXRHSs,
...
//                PredicateSet.indexProvider.size(), PredicateSet.indexProvider.toString());
        ...
        lattice.pruneLattice(invalidX, invalidXRHSs, validXRHSs, allPredicatesTemplate);
...
//        logger.info("Lattices Number : {}", workUnitLattices.size());
        ...
        Lattice nextLattice = sc.parallelize(workUnitLattices, workUnitLattices.size()).map(task -> {
            ...
            Lattice latticeWorker = task.generateNextLatticeLevelSampling(bLattice.getAllPredicates(), bLattice.getInvalidX(),bLattice.getInvalidRHSs(), bLattice.getValidXRHSs(),
                    bLattice.getSuppRatios(), bLattice.getPredicateProviderIndex(), bLattice.getOption(), null);
...
//                logger.info("***** Work Unit, {}", task);
                ...
                ArrayList<Predicate> currentNonPredicates = new ArrayList<>();
                ArrayList<Predicate> currentConstantTemplates = new ArrayList<>();
                for (Predicate p : task.getCurrrent()) {
                    if (p.isConstant()) {
                        currentConstantTemplates.add(p);
                    } else {
                        currentNonPredicates.add(p);
                    }
                }

                List<Message> messages = null;
                // 1. no constant predicates
                //if (currentConstantTemplates.size() == 0) {
                    ...
                    messages = multiTuplesRuleMining.validation_new(currentNonPredicates, task.getRHSs(), task.getPids());
...
//                            bobj.getInputLight(), bobj.getSupport(), bobj.getConfidence(), bobj.getMaxOneRelationNum(),
//                            task.getAllCount(), bobj.getTupleNumberRelations());
//                    ArrayList<Predicate> constantPredicatesPrefix = new ArrayList<>();
//                    for (Predicate rhs : task.getRHSs()) {
//                        REETemplate reeTemplate = new REETemplate(currentConstantTemplates, currentNonPredicates, rhs);
//                        List<Message> messages_ = multiTuplesRuleMining.validation(reeTemplate, task.getPids(), constantPredicatesPrefix);
...
//            }
    ...
    public void levelwiseRuleDiscoveryLocal() {
        ...
        for (Predicate p : this.allPredicates) {
            ...
            if (p.isConstant()) {
                if ( (!p.isConstantTemplate()) && p.getIndex1() == 0) {
                    applicationRHSs.add(p);
                }
            } else {
                applicationRHSs.add(p);
...
//            workUnits = this.solveSkewness(workUnits);
            ...
            // batch mode
            ArrayList<WorkUnit> workUnits_init = this.partitionWorkUnitsByRHSs(workUnits_);
            HashMap<PredicateSet, Double> currentSupports = new HashMap<>();
...
//            HashMap<IBitSet, Double> candSupports = this.getCandidateSupportRatios(messages);
            ...
            Lattice nextLattice = lattice.generateNextLatticeLevelSampling(this.allPredicates, this.invalidX, this.invalidXRHSs, this.validXRHSs,
                    currentSupports, predicateProviderIndex, option, null);
            // pruning
            nextLattice.removeInvalidLatticeAndRHSs(lattice);
            if (nextLattice == null || nextLattice.size() == 0) {
                break;
...
//                 ruleMessages.addAll(messages);
    ...
    private static Logger logger = LoggerFactory.getLogger(ParallelRuleDiscoverySamplingB.class);
