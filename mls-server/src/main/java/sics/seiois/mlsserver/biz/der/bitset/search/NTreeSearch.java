...
public class NTreeSearch implements ISubsetBackend, ITreeSearch {
    ...
    // rhs
    private IBitSet rhss = bf.create();


    public static BitSetFactory bf = new LongBitSet.LongBitSetFactory();
    /*
     * (non-Javadoc)
     *
    ...
    @Override
    public boolean add(IBitSet bs, IBitSet _rhss) {
        add(bs, _rhss, 0);
        return true;
    }

    private void add(IBitSet bs, IBitSet _rhss, int next) {
        int nextBit = bs.nextSetBit(next);
        if (nextBit < 0) {
            bitset = bs;
            rhss.or(_rhss);
        ...
        nextTree.add(bs, _rhss, nextBit + 1);
    }
    ...
    @Override
    public IBitSet containSubsetRHSs(IBitSet add) {
        IBitSet _rhss = bf.create();
        getSubsetRHSs(add, _rhss, 0);
        return _rhss;
    }

    private void getSubsetRHSs(IBitSet add, IBitSet _rhss, int next) {
        if (bitset != null && rhss != null) {
            _rhss.or(rhss);
        }
        int nextBit = add.nextSetBit(next);
        while (nextBit >= 0) {
            NTreeSearch subTree = subtrees.get(Integer.valueOf(nextBit));
            if (subTree != null) {
                subTree.getSubsetRHSs(add, _rhss, nextBit + 1);
            }
            nextBit = add.nextSetBit(nextBit +1);
        }
    }
